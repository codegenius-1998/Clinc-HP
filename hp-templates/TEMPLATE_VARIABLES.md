...
> このファイル名は初期実装（静的HTMLテンプレートを `variables.json` で変数化する方式）の名残です。その方式は廃止済みのため、本書は現行アーキテクチャの仕様書として読み替えてください。索引は [`AI_GUIDE.md`](AI_GUIDE.md)。

**2026年8月改訂: アーキテクチャを全面刷新しました。** 以前の版（静的HTMLテンプレートをコピーし、AIには空欄に入れる値だけを生成させる方式）はこのバージョンでは廃止されています。本書は現行の「OpenAIがページ内容を一から作成し、コードがHTML/CSSに組み立てる」方式について説明します。デザインプリセット抽出元だった `hp-templates/template0001`〜`template0006`（完成品HTMLテンプレート一式）は2026年8月に削除済みです。抽出済みの `presets/*.json` だけが残っています。
