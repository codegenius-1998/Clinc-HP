        ...
        {siteSections.map((ss) => (
          <div key={ss.id} className="rounded-2xl border border-slate-200 bg-white p-5">
            <div className="flex items-center justify-between gap-3">
              ...
              <form action={deleteSiteSectionAction.bind(null, ss.id, site.id)}>
                <button type="submit" className="text-[13px] text-slate-400 underline underline-offset-4 hover:text-red-600">
                  削除
                </button>
              </form>
            </div>
            <pre className="mt-3 overflow-x-auto rounded-lg bg-slate-50 p-3 text-[12px] text-slate-600">{ss.content}</pre>
          </div>
        ))}
