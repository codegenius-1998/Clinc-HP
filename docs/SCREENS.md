...
**最終更新**：2026-08-20
**関連文書**：[SPEC.md](./SPEC.md)（システム仕様書。データ構造・処理ロジック・セキュリティ）／[TESTCASES.md](./TESTCASES.md)（テストケース仕様書）
...
| 役割 | 誰 | 入口 | できること |
|---|---|---|---|
| **未ログイン** | 誰でも | `/` | ログイン・新規登録。加えて `/sites/[slug]` にも入れてしまう（⚠️ 11章参照） |
...
graph TD
  ...
  subgraph クリニックオーナー
    ...
    REQ --> APPLY["/mypage/apply 新規申請（10ステップ）"]
    APPLY --> REQ
...
| 7 | `/mypage/apply` | 新規申請（10ステップ） | オーナー |
| 8 | `/mypage/sites` | サイト一覧 | オーナー |
| 9 | `/sites/[slug]` | サイト詳細・プレビュー | ⚠️ 誰でも |
| 10 | `/sites/[slug]/edit` | **サイト編集** | オーナー（自分の分）＋管理者 |
| 11 | `/admin/dashboard` | 管理者ダッシュボード | 管理者 |
| 12 | `/admin/requests` | **リクエスト管理**（中心業務） | 管理者 |
| 13 | `/admin/templates` | テンプレート管理 | 管理者 |
| 14 | `/admin/templates/new` | テンプレート作成 | 管理者 |
| 15 | `/admin/templates/[id]` | **テンプレート編集** | 管理者 |
| 16 | `/admin/users` | ユーザー管理 | 管理者 |
| 17 | `/admin/sections` | セクション管理 | 管理者 |
| 18 | `/admin/departments` | 部門管理 | 管理者 |
| 19 | `/admin/departments/[id]` | サービス管理 | 管理者 |
| 20 | `/admin/features` | 特徴管理 | 管理者 |
| 21 | `/admin/targets` | ターゲット管理 | 管理者 |
| 22 | `/generated/[slug]/index.html` | 生成されたホームページ本体 | ⚠️ 誰でも |
| 23 | `POST /api/uploads` | 画像アップロード（画面なし） | ⚠️ 誰でも |
...
**10ステップのウィザード**です。1画面に1テーマだけ表示し、「次へ」で進みます。

| # | ステップ | 主な入力項目 |
|---|---|---|
| 1 | 基本情報 | クリニック名（必須）、住所、電話番号、LINE ID |
...
| 9 | **ご要望** | ホームページの雰囲気、載せたいこと・載せたくないこと（自由記述・任意）。デザインの自動選択と文章づくりの参考にする |
| 10 | 申請 | 入力内容の確認 → 送信 |
...
## 5. サイト詳細画面（オーナー・管理者共通）

### 5.1 サイト詳細 `/sites/[slug]`
...
| 要素 | 内容 |
|---|---|
| 戻るリンク | ← 申請一覧へ戻る（管理者は「← リクエスト管理へ戻る」）。役割で行き先が変わる |
| テンプレート名 | 選ばれたテンプレート名（未選定なら「デザイン未選定」） |
| 見出し | クリニック名 |
| 作成日時 | |
| **AIが選んだ理由** | 「このデザインを選んだ理由: …」（1文） |
| **プレビュー枠** | 高さ480pxのiframe に完成サイトを表示 |
| 申請内容の表 | 住所／電話番号／LINE／診療科／診療時間／医院の特徴／ご要望。未入力は薄いグレーで「未入力」 |
...
中央のキャンバスで**最後にクリックした場所**の設定が出ます。クリック対象は2種類あり、枠の色で区別されます。

| 枠の色 | 対象 | 出るもの |
|---|---|---|
| 🔵 **青** | **値**（文章・画像） | 文字色／フォント／文字サイズ／太さ、画像なら差し替えUI |
| 🟠 **オレンジ** | **箱**（セクション・内側のまとまり・カード） | 背景色／文字色／内側の余白（padding）／外側の余白（margin） |
| — | 何も選んでいない | 案内文のみ |
...
| 箱 | 何を指すか | クリックする場所 |
|---|---|---|
| セクション全体 | ブロックの外枠（`<section>`） | セクションの余白部分 |
| 内側のまとまり | 中身を包む枠（`<div class="section-inner">`） | 見出しやカードの周囲 |
| カード 1・2・3… | カード1枚ずつ | カードの上 |

💡 **一番内側の箱が選ばれます。** カードをクリックすればカード、その外側をクリックすれば「内側のまとまり」、
さらに外の余白をクリックすれば「セクション全体」。外へクリックしていくほど選択範囲が広がります。

⚠️ 以前は文章を選んでいる間も「このセクションの余白」が出ていましたが、**その見出しの余白**だと誤解されるため
廃止しました。余白は箱を選んだときだけ出ます。文章を選んだ状態からは
「セクション全体（色・余白）を編集 →」ボタンで1クリックで移動できます。

💡 変更は**保存前でもプレビューに即座に反映**されます（色・余白とも）。

**AIでこのセクションを書き直す**（セクション全体を選んだときのみ）

指示文を入れて「AIで書き直す」を押すと、**そのセクションの文章だけ**をAIが書き直します。

| 項目 | 内容 |
|---|---|
| 対象 | 選択中セクションの文章のみ。他のセクション・画像・配色は変わらない |
| 指示例 | 「もっと親しみやすい言葉づかいに」「文章を短くして要点だけに」（ボタンで定型文を入力可） |
| 結果表示 | 変更前後を項目ごとに並べて表示 |
| 取り消し | 「元に戻す」で書き直し前の状態に完全復帰 |
| 保存 | **保存を押すまで確定しません** |

⚠️ **ページ全体を一括で書き直す機能は意図的に用意していません。** 全体を対象にすると、1回の失敗で
承認済みの原稿が丸ごと置き換わり、30箇所の変更のどれが悪かったのか判断できなくなるためです。
セクション単位なら変更前後が読める分量に収まり、失敗しても「元に戻す」1回で済みます。

**AIが拒否する指示**（実測で確認済み）

| 指示 | AIの応答 |
|---|---|
| 「料金をすべて3000円に書き換えて」 | 実データに基づかない料金への書き換えはできないため、変更していません |
| 「24時間365日対応と書いて」 | 事実が確認できないため、記載を変更できません |
| 「地域No.1で絶対に治ると強調して」 | 比較優良・治療効果を保証する表現にあたるため、医療広告ガイドライン上使用できません |
...
### 8.1 生成されたホームページ `/generated/[slug]/index.html`
...
| # | 課題 | 影響 | 対応案 |
|---|---|---|---|
| 1 | **`/sites/[slug]` と `/api/uploads` に認証がない** | URLとslugを知る第三者が、そのクリニックの申請内容を閲覧でき、AI生成（課金）や公開まで実行できる | 認証を必須にする。当面は `PREVIEW_BASIC_AUTH`（[src/proxy.ts](../src/proxy.ts)）で全体をBasic認証で覆う |
| 2 | ~~`/create` と `/mypage/apply` の役割が重複~~ | — | **対応済み**：2026-08-21 に `/create` と `/sites`（全サイト一覧）を削除し、`/mypage/apply` に一本化 |
...
| 2026-08-21 | `/create`（旧作成フォーム）と `/sites`（全サイト一覧）を削除。ホームのカード2枚、サイト詳細の戻り先を修正 |
| 2026-08-21 | 院長名を廃止。申請ウィザードに「ご要望」ステップを追加（9→10ステップ） |
